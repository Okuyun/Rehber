        ...
        ayas.forEach(
            (words,indA) => {
              ...
              let temp = new Map(rootsVector);
               array.forEach( e=> { temp.set(wordToRoot.get(toBuckwalter(e)),temp.get(wordToRoot.get(toBuckwalter(e)))+1)})
...
function innerProd(a,b){
    let av=[...a.values()]
    let bv=[...b.values()]
    let reducer = (a,curr,ind)=> curr?a+= curr*bv[ind]:a;
    return av.length<bv.length? av.reduce(  reducer,0 ):bv.reduce(reducer,0);
}
